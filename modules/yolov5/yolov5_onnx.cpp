...
namespace yolov5 {
    Net::Net(const std::string &onnx_model_path, const std::string &class_list_path, const float resolution_,
             bool is_cuda) {
        // load models
        ...
        this->output_dimensions = 5 + static_cast<int>(this->class_list.size());
        this->output_rows = static_cast<int>(3 *(pow(this->resolution/8,2) + pow(this->resolution/16,2) + pow(this->resolution/32,2)));
    }

    std::vector<yolov5::Detection> Net::detect(cv::Mat &src, float _score_threshold, float _nms_threshold, float _confidence_threshold) {
        cv::Mat blob;
        ...
        float x_factor = static_cast<float >(input.cols) / this->resolution;
        float y_factor = static_cast<float >(input.rows) / this->resolution;
        ...
        for (int i = 0; i < this->output_rows; ++i) {
            ...
            if (confidence >= _confidence_threshold) {
                ...
                cv::Mat scores(1, static_cast<int>(this->class_list.size()), CV_32FC1, classes_scores);
                cv::Point class_id;
